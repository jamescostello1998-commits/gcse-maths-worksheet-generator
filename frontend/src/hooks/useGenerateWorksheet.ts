...
export function useGenerateWorksheet(): UseGenerateWorksheetResult {
  ...
  const generate = useCallback(
    async (topicId: string, tier: Tier, options: WorksheetOptions = {}) => {
      setStatus('loading')
      setError(null)
      try {
        const blob = await generateWorksheet(topicId, tier, { ...options, format })
        const suffix = options.answersOnly ? '-answers-only' : ''
        const ext = format === 'docx' ? 'docx' : 'pdf'
        triggerDownload(blob, `${topicId}-${tier}-worksheet${suffix}.${ext}`)
        setStatus('success')
      } catch (err) {
