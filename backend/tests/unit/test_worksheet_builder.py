...
def test_happy_path_produces_20_distinct_questions():
    worksheet = builder.build_worksheet("linear_equations", Tier.FOUNDATION, rng=random.Random(1))
    assert len(worksheet.questions) == 20
    assert len({q.dedup_key for q in worksheet.questions}) == 20
    assert worksheet.topic_id == "linear_equations"
    assert worksheet.tier == Tier.FOUNDATION


def test_all_topics_and_tiers_produce_20_distinct_questions():
...
def test_exhaustion_raises_worksheet_generation_error(monkeypatch):
    ...
    fake_topic = TopicDefinition(id="fake", display_name="Fake", description="", generate=limited_generate)
